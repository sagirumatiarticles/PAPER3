...
output:
 bookdown::word_document2:
    reference_docx: REF_DOCX/paper3_template.docx
...
knitr::opts_chunk$set(echo = FALSE,comment=NULL,warning=F)
...
```{r eval=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx",docx(),latex()),results='asis'}
if(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx"){cat("#### Abstract")} else {cat("\\centerline{\\bf{Abstract}}")
}
```  
...
```{r eval=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx",docx(),latex()),results='asis'}
if(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx"){cat("***")} else {cat("\\hrulefill")
}
```  
...
For the full sample of 1975:05 to 2018:08, all the inflation variables (GI, RI and DI) appear to be I(0) except the global inflation, Nigerian inflation and Gambian inflation, based on formal unit root tests developed by @10.2307/2336182 and @Dickey1979. However, when structural breaks are taken into account, these integrated inflation variables become stationary. The structural breaks occurs at various dates, so a dummy variable is generated to capture each break date for each inflation variable, 1 is assigned for the specific break date and 0 otherwise^[The SB dates for the global inflation is 1981:11, and 1987:01 and 1996:07 for the Gambia and Nigeria respectively]. Formally $D_{it}=1$ for $t=i$ and $D_{it}=0$ for $t\neq i$, where $i$ is the break date.  For the sake of comparison, four subsamples are considered; two samples capturing the period before and after establishment of WAEMU and other two samples covering the period before and after creation of WAMZ. New definition of global CPI, which includes China, is also used for the samples after creation of WAEMU and WAMZ because China becomes a major global player in recent years.  VAR with two lags is estimated for all the countries in the form of equation \@ref(eq:reduced-VAR)^[The lag selection is guided by the number of lags necessary to make the VAR stable and whiten its error.] . All the VAR models meet the diagnostic requirements of homoscedasticity, non-autocorrelation and stability^[The diagnostic results are not reported in order to save space.]. The models used in this study are defined in Table \@ref(tab:tabDef)^[To save space, only comparison of models 2 and 3 and that of models 5 and 6 are reported. We also determine that inclusion of China as part of the global variable does make any significant difference.]

```{r tabDef,echo=F}
tabDef=read.csv("TABLES/table1.csv",allowEscapes = T,header = T,check.names = FALSE)  
tabDefKable=kable(tabDef,format=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx","pandoc","latex"),booktabs=T,caption="Sample, Model and Definition of Global Variable",escape=F,linesep="") 
```

```{r tabDef-latex, eval=latex()}
tabDefKable=kable_styling(tabDefKable,latex_options = c("basic","hold_positon","scale_down")) %>%
  footnote(general="Note: a, b, c, d, and e stand for full sample, period before WAEMU, period after WAEMU,  period before WAMZ and period after WAMZ respectively.", general_title = "",footnote_as_chunk=T,threeparttable=T,escape=F) %>% 
  row_spec(0,bold=T)
tabDefKable
```

```{r tabDef-docx, eval=docx()}
tabDefKable
y="Note: a, b, c, d, and e stand for full sample, period before WAEMU, period after WAEMU,  period before WAMZ and period after WAMZ respectively."
...
```{r decompM2M3,echo=F}
decompM2M3=read.csv("TABLES/table2.csv",allowEscapes = T,header = T,check.names = FALSE,colClasses = "character")  
decompM2M3Kable=kable(decompM2M3,format=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx","pandoc","latex"),booktabs=T,caption="Variance decomposition of inflation, M2 and M3",escape=F,linesep="") 
```

```{r decompM2M3-latex, eval=latex()}
decompM2M3Kable=kable_styling(decompM2M3Kable,latex_options = c("basic","hold_positon","scale_down")) %>%
row_spec(0,bold=T) %>% 
  add_header_above(c(" "=1,"2-month horizon (M2)"=3,"2-month horizon (M3)"=3,"24-month horizon (M2)"=3,"24-month horizon (M3)"=3),bold=T)
decompM2M3Kable
```
  
```{r decompM2M3-docx, eval=docx()}
decompM2M3Kable
```

 
To make comparison between the period before and after the birth of WAMZ, Table \@ref(tab:decompM5M6) reports the variance decomposition of M5 and M6. According to M6-2H and M6-24H, Ghana, the Gambia and Nigeria are still not RS-dominated even after the creation of WAMZ, as their RS magnitudes are 45.70, 0.49 and 1.71 (2-month horizon) and 26.94, 0.30 and 5.78 (24-month horizon). This means that Anglophone countries are not ready for the common currency.



```{r decompM5M6,echo=F}
decompM5M6=read.csv("TABLES/table3.csv",allowEscapes = T,header = T,check.names = FALSE,colClasses = "character")  
decompM5M6Kable=kable(decompM5M6,format=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx","pandoc","latex"),booktabs=T,caption="Variance decomposition of inflation, M5 and M6",escape=F,linesep="") 
```

```{r decompM5M6-latex, eval=latex()}
decompM5M6Kable=kable_styling(decompM5M6Kable,latex_options = c("basic","hold_positon","scale_down")) %>%
row_spec(0,bold=T) %>% 
add_header_above(c(" "=1,"2-month horizon (M5)"=3,"2-month horizon (M6)"=3,"24-month horizon (M5)"=3,"24-month horizon (M6)"=3),bold=T)

decompM5M6Kable
```
```{r decompM5M6-docx, eval=docx()}
decompM5M6Kable
```
...
```{r corM2M3,echo=F}
corM2M3=read.csv("TABLES/table4.csv",allowEscapes = T,header = T,check.names = FALSE,colClasses = "character")  
corM2M3Kable=kable(corM2M3,format=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx","pandoc","latex"),booktabs=T,caption="Correlation of inflationary shocks: M2 and M3.",escape=F,linesep="") 
```

```{r corM2M3-latex, eval=latex()}
corM2M3Kable=kable_styling(corM2M3Kable,latex_options = c("basic","hold_positon","scale_down")) %>%
row_spec(0,bold=T) %>% 
  pack_rows("Panel A: Correlation of domestic inflationary shocks (M2)",0,9,hline_after=T,indent=FALSE) %>%
  pack_rows("Panel B: Correlation of domestic inflationary shocks (M3)",9,9,hline_after=T,latex_align = "l",indent=FALSE,hline_before=T) %>%
  row_spec(9,bold=T) 
corM2M3Kable
```

```{r corM2M3-docx, eval=docx()}
corM2M3Kable
```
  


Table \@ref(tab:corM5M6) is a report about the correlation estimates of M5 and M6 in order to compare the period before and after the birth of WAMZ. It is clear that creation of WAMZ does not lead to the symmetry of shocks across the ECOWAS countries as there is no strongly positive correlation estimate for model M6 . Even considering China as one the global players does not make the domestic shocks symmetric as none of the correlation estimates of M7 is strongly positive.
 

```{r corM5M6,echo=F}
corM5M6=read.csv("TABLES/table5.csv",allowEscapes = T,header = T,check.names = FALSE,colClasses = "character")  
corM5M6Kable=kable(corM5M6,format=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx","pandoc","latex"),booktabs=T,caption="Correlation of inflationary shocks: M5 and M6.",escape=F,linesep="") 
```

```{r corM5M6-latex, eval=latex()}
corM5M6Kable=kable_styling(corM5M6Kable,latex_options = c("basic","hold_positon","scale_down")) %>%
row_spec(0,bold=T) %>% 
  pack_rows("Panel A: Correlation of domestic inflationary shocks (M5)",0,9,hline_after=T,indent=FALSE) %>%
  pack_rows("Panel B: Correlation of domestic inflationary shocks (M6)",9,9,hline_after=T,latex_align = "l",indent=FALSE,hline_before=T) %>%
  row_spec(9,bold=T) 
corM5M6Kable
```
```{r corM5M6-docx, eval=docx()}
corM5M6Kable
```
...
```{r eval=ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx",docx(),latex()),results='asis'}
if(knitr::opts_knit$get("rmarkdown.pandoc.to")=="docx"){cat("##### References")} else {cat("# References{-}")
}
```  


```{r eval=latex(),results='asis'}
cat("\\noindent")
cat("\\leftskip1cm")
cat("\\parindent-1cm")
```  
